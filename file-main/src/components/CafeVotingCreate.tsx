...
interface CafeVotingCreateProps {
  user: any;
  meetupData: any;
...
export default function CafeVotingCreate({ user, meetupData, onNavigate, onBack }: CafeVotingCreateProps) {
  ...
  useEffect(() => {
    if (meetupData._id) {
      ...
      const handleVoteUpdate = (data: any) => {
        ...
        // Auto navigate if a cafe has been selected
  ...
  const handleVote = async (cafeId: string) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/meetups/vote', {
        method: 'POST',
      ...
      {myVote && (
        <div className="bg-gradient-to-r from-green-500 to-emerald-500 py-4 px-6">
          <div className="max-w-5xl mx-auto flex items-center justify-center gap-3">
            ...
            <p className="text-white font-bold text-lg">Your vote as admin has been recorded!</p>
          </div>
        </div>
      )}
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="bg-gradient-to-br from-[#f5ebe0] to-[#e3d5ca] rounded-2xl p-6 mb-8 border border-[#be9d80]">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-gray-700 mt-1">
                {formatDate(meetupData.date)} • {formatTime(meetupData.time)}
              </p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-4 pt-4 border-t border-[#be9d80]/30">
            ...
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-gray-900">{totalMembers - totalVoted}</p>
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-2xl p-5 mb-8">
          <div className="flex gap-4">
            ...
            <div>
              <h3 className="font-bold text-gray-900 mb-1">Cast Your Vote as Admin</h3>
              <p className="text-sm text-gray-700">
                You selected {meetupData.selectedCafes.length} cafés for your group. Now vote for your favorite! The café with the most votes will be selected.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {meetupData.selectedCafes.map((cafe: any) => {
            ...
            const votePercentage = totalVoted > 0 ? Math.round((cafeVoteCount / totalVoted) * 100) : 0;
